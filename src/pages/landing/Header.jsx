...
export default function Header({ user, onLoginClick, onRegisterClick, t }) {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
 
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      <nav className="mt-5 mx-4 max-w-7xl lg:mx-auto flex items-center justify-between px-4 lg:px-10 py-4 bg-white/95 backdrop-blur-md supports-[backdrop-filter]:bg-white/85 border border-slate-200/70 ring-1 ring-black/5 shadow-md rounded-full">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold tracking-tight">Note Pro Meeting</span>
          ...
          {user ? (
            ...
            <>
              <button
                ...
                className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-xl font-semibold bg-white text-slate-900 border border-slate-200 hover:bg-slate-50 active:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-cyan-300"
              >
                {t('nav.login')}
              </button>
              <button
       ...
       {mobileMenuOpen && (
          <div className="mt-38 md:hidden bg-white border-x border-b border-slate-200 shadow-lg rounded-b-2xl overflow-hidden">
            <div className="px-4 py-4 space-y-3">
              <a className="block py-2 text-base text-slate-700 hover:text-blue-700 hover:bg-blue-50 rounded-md px-4 font-medium transition-colors" href="#features" onClick={() => setMobileMenuOpen(false)}>{t('nav.features')}</a>
              ...
              <div className="pt-3 border-t border-slate-200 space-y-2">
                <button
                  ...
                  className="w-full justify-center inline-flex items-center gap-2 px-4 py-2 rounded-xl font-semibold bg-white text-slate-900 border border-slate-200 hover:bg-slate-50">
                    {t('nav.login')}
                </button>
                <button
                  onClick={() => {
